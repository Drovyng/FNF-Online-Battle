...
class EditorPlayState extends MusicBeatState
{
	...
	{
		FlxG.sound.playMusic(Paths.inst(Conductor.SONG.song), 0, false);
					...
					if(daStrumTime >= startPos) {
						...
						if(floorSus > 0) {
							for (susNote in 0...floorSus+1)
							{
								...
								var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(Conductor.SONG.speed, 2)), daNoteData, oldNote, true);
	...
	override function update(elapsed:Float) {
		...
		if (unspawnNotes[0] != null)
		{
			...
			if(Conductor.SONG.speed < 1) time /= Conductor.SONG.speed;
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if(daNote.copyY) {
					if (ClientPrefs.downScroll) {
						daNote.y = (strumY + 0.45 * (Conductor.songPosition - daNote.strumTime) * Conductor.SONG.speed);
						if (daNote.isSustainNote) {
							//Jesus fuck this took me so much mother fucking time AAAAAAAAAA
							if (daNote.animation.curAnim.name.endsWith('end')) {
								daNote.y += 10.5 * (fakeCrochet / 400) * 1.5 * Conductor.SONG.speed + (46 * (Conductor.SONG.speed - 1));
								daNote.y -= 46 * (1 - (fakeCrochet / 600)) * Conductor.SONG.speed;
							...
							daNote.y += (Note.swagWidth / 2) - (60.5 * (Conductor.SONG.speed - 1));
							daNote.y += 27.5 * ((Conductor.SONG.bpm / 100) - 1) * (Conductor.SONG.speed - 1);
				...
				if (Conductor.songPosition > (noteKillOffset / Conductor.SONG.speed) + daNote.strumTime)
				{
					if (daNote.mustPress)
					{
